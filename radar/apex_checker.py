...
# ═══════════════════════════════════════════════════════════════════════
# ApeX Checker v3.2 — REGISTRY DRIVEN
# ═══════════════════════════════════════════════════════════════════════
...
async def refresh_apex() -> dict:
    ...
    return {"ok": False, "source": "error", "count": 0}
...
def get_apex_info(symbol: str) -> ApexInfo:
    ...
    if not t:
        return ApexInfo(sym, False, 0, 0, 0, 0, 3600, "none")
...
def format_apex_block(symbol: str, long_ex: str, long_rate_h: float, size_usd: float = 50) -> str:
    ...
    if not info.available: return f"🔷 ApeX: {symbol} — данных нет"
    
    r = info.funding_rate_h; r_str = f"{r:+.6f}%/ч"
...
async def cmd_apex(update, context):
    if not context.args: await update.effective_message.reply_text("Использование: `/apex SYM`")
    else: 
        sym = context.args[0].upper()
        if not _store.is_fresh():
            await update.effective_message.reply_text(f"⏳ Обновляю реестр ApeX...")
            await refresh_apex()
        await update.effective_message.reply_text(format_apex_block(sym, "okx", -0.01), parse_mode="Markdown")
